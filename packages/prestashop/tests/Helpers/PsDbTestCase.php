...
abstract class PsDbTestCase extends TestCase
{
    protected static \mysqli $sharedMysqli;

    protected MysqliPsDb $db;
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        if ($mysqli->connect_errno !== 0) {
            static::markTestSkipped(
                'MySQL not available for native PS DB tests: '.$mysqli->connect_error,
            );
    ...
    protected function setUp(): void
    {
        ...
        if (! isset(static::$sharedMysqli)) {
            self::markTestSkipped('MySQL not available for native PS DB tests.');
        }

        $this->prefix = defined('_DB_PREFIX_') ? (string) constant('_DB_PREFIX_') : 'ps_';
        $this->db = new MysqliPsDb(static::$sharedMysqli);
        $this->managedTables = [];
    }
